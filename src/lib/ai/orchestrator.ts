...
const CLARIFICATION_AGENT_PROMPT = `You are a specialized clarification agent for scope creep pricing. Your job is to identify gaps in provided information and generate targeted questions.

## Information Requirements

### REQUIRED (Must Have)
- Location: City, State/Country - affects labor rates
- Original Scope: Clear description of contracted work
- New Request: What is being added/changed
- Project Type: Industry/domain
- Original Value: Contract amount for proportionality

### HIGHLY RECOMMENDED
- Timeline: Original deadline, current status
- Skill Types: Specific trades/roles involved
- Urgency: Is this a rush request?
...
4. Focus on what impacts pricing most
...
export async function generateClarificationQuestions(
  ...
  const rulesContext = `
## Project Rules
${rules.deliverables ? `- Deliverables: ${(rules.deliverables as string[]).join(', ')}` : ''}
${rules.hourlyRate ? `- Hourly Rate: $${rules.hourlyRate}/hr` : ''}
${rules.revisionsIncluded ? `- Revisions Included: ${rules.revisionsIncluded}` : ''}
${rules.rulesSummary ? `- Summary: ${rules.rulesSummary}` : ''}
`;
...
Generate 3-4 clarifying questions to help understand and price this request. Focus on:
- Scope/extent of work
- Technical requirements
- Timeline/urgency
- Any unclear deliverables`;
